...
# With interaction
m1<- brm(PU |trunc(lb=3) ~ mean_age + (1|site),  
           family = poisson(),
           data = d,
           iter = 10000, warmup = 5000,
           prior = c(set_prior("normal(2, 0.5)", class = "Intercept"),
...
m2<- brm(PU |trunc(lb=3) ~ s(mean_age, k = 14) + (1|site),
         family = poisson(),
         data = d,
         iter = 10000, warmup = 5000,
         prior = c(set_prior("normal(2, 0.5)", class = "Intercept"),
...
m2_early<- brm(PU |trunc(lb=3) ~ s(early_age, k = 14) + (1|site),
         family = poisson(),
         data = d,
         iter = 10000, warmup = 5000,
         prior = c(set_prior("normal(2, 0.5)", class = "Intercept"),
...
m2_late<- brm(PU |trunc(lb=3) ~ s(late_age, k = 14) + (1|site),
               family = poisson(),
               data = d,
               iter = 10000, warmup = 5000,
               prior = c(set_prior("normal(2, 0.5)", class = "Intercept"),
...
m2_priors<- brm(PU |trunc(lb=3) ~ s(mean_age, k = 14) + (1|site),
         family = poisson(),
         data = d,
         iter = 10000, warmup = 5000,
         prior = c(set_prior("normal(2, 0.5)", class = "Intercept"),
